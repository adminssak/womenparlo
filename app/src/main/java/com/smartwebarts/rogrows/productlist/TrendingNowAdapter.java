...
public class TrendingNowAdapter extends RecyclerView.Adapter<TrendingNowAdapter.MyViewHolder>{
    ...
    public TrendingNowAdapter(Context context, List<ProductModel> list) {
    ...
    @NonNull
    @Override
    public TrendingNowAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new TrendingNowAdapter.MyViewHolder(LayoutInflater.from(context).inflate(R.layout.newdesign, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull TrendingNowAdapter.MyViewHolder holder, final int position) {

        try {
//            if (removeId.equalsIgnoreCase(list.get(position).getId())) {
//                holder.minteger = 1;
...
//            }
            ...
            holder.txtQuan.setText(String.format("%s", 0));
